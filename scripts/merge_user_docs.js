 ...
 * - Copies all subcollections (doc-by-doc) from source -> target
 * - Optionally deletes the source subcollections and source document
 *
 * Usage:
 *   node merge_user_docs.js <SOURCE_USER_ID> <TARGET_USER_ID> [--dry-run] [--no-delete]
 *
 * Auth/Creds:
 * - Uses Application Default Credentials. Either:
 *   1) Run `gcloud auth application-default login`, or
 *   2) Set env var GOOGLE_APPLICATION_CREDENTIALS to a service account JSON path.
 */
...
async function main() {
  ...
  if (!sourceUserId || !targetUserId) {
    console.error('Usage: node merge_user_docs.js <SOURCE_USER_ID> <TARGET_USER_ID> [--dry-run] [--no-delete]');
    process.exit(1);
  }
