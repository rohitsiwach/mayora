...
class _InvitationSignUpPageState extends State<InvitationSignUpPage> {
  ...
  Future<void> _completeRegistration() async {
    ...
    try {
      ...
      String? userId;
      if (current != null && (current.email ?? '').toLowerCase() == invitedEmail.toLowerCase()) {
        userId = current.uid;
      } else {
        ...
        if (exists) {
          // Guide user to sign in instead of creating a duplicate account
          if (mounted) {
            await showDialog(
              context: context,
                ...
                actions: [
                  ...
                  ElevatedButton(
                    onPressed: () {
                      ...
                      // Navigate to the auth wrapper which shows Sign In if not authenticated
